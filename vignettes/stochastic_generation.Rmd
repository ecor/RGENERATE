...
The rationale of this vignette is the presentation of a stochastic generation of daily mininum and maximum tempareture in two or more sites meke use of **RMAWGEN** and **RGENERATE** packages. The illustrated procedure refers to *trentino* dataset (**RMAWGEN** package), ad after loading the necessary packages and calling the  *trentino* dataset (**RMAWGEN** package), is organizede as follows: 
...
- Calibration of the stochastic genarations, i.e. creation of a *GPCAvarest2* object instance for the model whose parameterts like order of autocorrations and number of GPCA itarations are inserted as function arhguments;
- Usage of the model to generate a new time series of tmeparature anomalies. 

This tutorial aims at generating a time series of temperature anomalies stating from a time series of observations. The observations were cleaned from interpolations of the averages of each month of the year reperesenting tha actual climate conditions. The generated time series mantains the same statistical properties of the one of the observed anomalies. The anomalies can be added to different scanarios of climate conditions. 
